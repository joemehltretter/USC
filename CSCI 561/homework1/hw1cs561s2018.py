...
def main():
  ...
  scoreBoard = gameInfo[11]
  score = []
  for val in scoreBoard.split("'"):
    for obj in val.split(','):
      score.append(obj)
  for count in range(len(score)):
    BoardConfig.circScore[count] = np.asarray(score[count])
    BoardConfig.starScore[7-count] = np.asarray(score[count])
  print BoardConfig.circScore
  print BoardConfig.starScore
